.PHONY: build clean e2e-test install integration-test lint re test tool

BINARY	=	opentofu-kms-ovhcloud
BIN_DIR	=	$(CURDIR)/bin
BUILD_DIR = $(CURDIR)/build

CMD_PATH	=	./cmd/opentofu-kms-ovhcloud
...
e2e-test: build
	...
	@BIN=$(BUILD_DIR)/$(BINARY) ./scripts/run-e2e.sh
...
integration-test: tool
	...
	@mkdir -p $(BUILD_DIR)
	$(GO) test -race -v -json -coverprofile=$(BUILD_DIR)/integration-coverage.out --tags=integration ./... > $(BUILD_DIR)/integration-test-results.json
...
test: tool
	@echo " > Running tests..."
	@mkdir -p $(BUILD_DIR)
	$(GO) test -race -v -json -coverprofile=$(BUILD_DIR)/coverage.out ./... > $(BUILD_DIR)/test-results.json
